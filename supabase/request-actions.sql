...
BEGIN
  ...
  IF (SELECT count(*) FROM pg_catalog.pg_policies
    ...
    )) <> pg_catalog.cardinality(policy_names) THEN
...
REVOKE UPDATE, TRIGGER, TRUNCATE, MAINTAIN ON public.requests FROM PUBLIC, anon, authenticated;
...
BEGIN
  IF pg_catalog.has_table_privilege('anon', 'public.requests', 'SELECT,INSERT,UPDATE,DELETE,TRIGGER,TRUNCATE,MAINTAIN')
    OR pg_catalog.has_table_privilege('authenticated', 'public.requests', 'UPDATE,TRIGGER,TRUNCATE,MAINTAIN')
